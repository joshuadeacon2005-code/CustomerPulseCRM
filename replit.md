...
- **Analytics**: Advanced analytics with **monthly and overall views**. Monthly analytics reset at the start of each month showing current month data only (customers created this month, interactions this month). Overall analytics display all-time data. Features include team structure visualization, team performance metrics (customer counts, targets, to-dos), and user ID to name mapping for improved readability across charts.
