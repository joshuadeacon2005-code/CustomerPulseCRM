...
- **Role System Overhaul**: Complete restructuring of user roles with 5-tier hierarchy (CEO > Sales Director > Regional Manager > Manager > Salesman). The Admin role has been renamed to Sales Director, and Regional Manager role added between Manager and Sales Director. All authentication middleware, role guards, and UI references updated to reflect new hierarchy. **Backward Compatibility**: Authorization middleware accepts both "admin" (legacy) and "sales_director" (current) role names to support existing users.
