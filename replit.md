...
- **Customer Management**: Comprehensive profiles with country tracking, main and additional contacts (with full edit/delete capabilities), notes, Bloom Connect marketplace integration, store address, structured retailer type selection (14 categories), quarterly soft targets (with currency support), last contact date, lead management (date of first contact, lead source), and interaction tracking. Advanced filtering by brand and retailer type. View toggle allows switching between card/grid and table/list layouts with localStorage persistence across sessions. External system integration with optional NetSuite and BloomConnect URL fields, featuring prominent link buttons in customer detail modal that open URLs in secure new tabs (with noopener/noreferrer protection). **Multiple Addresses**: Full support for multiple addresses per customer with dedicated Addresses tab in customer detail modal. Each address includes address type (Store, Office, Warehouse, Distribution Center, Other), primary address field, optional Chinese address translation field, and additional translation notes. Features complete CRUD operations (add, edit, delete) with Drizzle ORM persistence and chronological sorting (newest first). Supports bulk address import via CSV with automatic assignment to correct customer records.
